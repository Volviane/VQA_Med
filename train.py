...
def train_model(config): #  model, criterion, optimizer, scheduler, data_loader, batch_size, num_epochs=25, alpha=.1, gamma=2): #gamma = 0.5 with the actual alpha =.25, #alpha = 0.5, 0.1 with the actual gamma=2
    since = time.time()
    ...
    # print('heeeeeeeeeeeeeeeeeeeeeeeeeee----------------------------===================',opt.NUM_OUTPUT_UNITS)

    model = VqaClassifierModel( opt=opt ).to(device)

    criterion = nn.CrossEntropyLoss() #weight=weights.to(device)
    optimizer = optim.Adam(model.parameters(), lr=opt.INIT_LERARNING_RATE, weight_decay=opt.LAMNDA) #lr=opt.INIT_LERARNING_RATE, weight_decay=opt.LAMNDA   lr=config["lr"], weight_decay=config["lamnda"]
    ...
    num_workers = 0
    batch_size = opt.BATCH_SIZE #int(config["batch_size"]) 
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'valid']:
            ...
            if phase =='train':
                ...
                # tune.report(loss=epoch_loss, accuracy=epoch_acc1)
...
def main(num_samples=10, max_num_epochs=200, gpus_per_trial=1):
    ...
    # input_dir = config.input_dir #'/home/smfogo/Med_Visual_question_answering/Exp1_4'#'/content/Med_Visual_question_answering/Exp1_3/'#'./'
    # input_vqa_train =config.input_vqa_train # 'train_dataset_pickle/train_dataset_df.pkl'
    # input_vqa_valid =config.input_vqa_valid#'valid_dataset_pickle/valid_dataset_df.pkl'

    # img_feat_train = config.img_feat_train#'train_dataset_pickle/train-image-feature.pickle'
    # img_feat_valid =config.img_feat_valid#'valid_dataset_pickle/valid-image-feature.pickle'
    ...
    #     "lamnda":tune.loguniform(1e-4, 1e-1)
    # },
    # num_samples=2,)
    ...
    # batch_size =  opt.BATCH_SIZE #32 #32 #16 config["batch_size"]#

    # print(batch_size)

    # num_workers = 0

    # learning_rate = opt.INIT_LERARNING_RATE#1e-4#0.001# 0.000001 config["lr"]#opt.INIT_LERARNING_RATE#1e-4#0.001# 0.000001
    # lamnda = opt.LAMNDA#1e-3 config["lamnda"]#opt.LAMNDA#1e-3
    # print(learning_rate)
    # print(lamnda)
    # #step_size = 7
    #gamma = 0.9
    # momentum = opt.MOMENTUM #0.9
    # num_epochs =opt.NUM_EPOCHS #60 #20  #100 # 300

    
    # image_size = opt.IMG_INPUT_SIZE#224
    # num_class = len(answer_classes )    #1552  #44#15#10#1485
    ...
    # model = VqaClassifierModel( opt=opt ).to(device)
    # model.qst_vocab_size = 10000
    # print(model.qst_vocab_size)

    # criterion = nn.CrossEntropyLoss() #weight=weights.to(device)
    # criterion = WeightedFocalLoss()
    # optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=lamnda) #weight_decay=lamnda
    # optimizer = optim.AdamW(model.parameters(), lr=learning_rate) # , betas=(0.9, 0.999), eps=1e-08, amsgrad=False, weight_decay=lamnda #0.55475 without weight_decay
    # optimizer = optim.SGD(model.parameters(), 
    #                      lr=learning_rate, momentum=momentum)
    # Decay LR by a factor of 0.1 every 7 epochs
    # exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=12, gamma=0.1)  #0.5661% with step size of 2 # 0.5648 with step size of 4
    # train_model(model, criterion, optimizer, exp_lr_scheduler, data_loader,batch_size, num_epochs=num_epochs)
    ...
    config = {
        ...
        # "lamnda": tune.loguniform(1e-4, 1e-2),
