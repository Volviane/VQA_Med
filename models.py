...
#     path_output_chd ='/home/smfogo/Med_Visual_question_answering/Exp1_4'#'/content/Med_Visual_question_answering/Exp1_3'#'.'

#     train_vqa = np.load(path_output_chd+'/train_dataset_pickle/train_dataset_df.pkl', allow_pickle=True )
#     # train_vqa = pd.read_csv(path_output_chd+'/train_dataset_pickle/train_dataset_df.csv')#.gz
#                     #  , sep=','
#                     # , header=True
#                     # , index=False
#                     # , chunksize=100000
#                     # , compression='gzip'
#                     # , encoding='utf-8')
#     val_vqa = np.load(path_output_chd+'/valid_dataset_pickle/valid_dataset_df.pkl', allow_pickle=True )

#     # val_vqa = pd.read_csv(path_output_chd+'/valid_dataset_pickle/valid_dataset_df.csv.gz')
#                     #  , sep=','
#                     # , header=True
#                     # , index=False
#                     # , chunksize=100000
...
#     all_questions = np.concatenate([train_vqa['Question'].values, val_vqa['Question'].values])
    
#     # Encode our concatenated data
#     encoded_questions = [tokenizer.encode(sent, add_special_tokens=True) for sent in all_questions]

#     # Find the maximum length
#     max_len = max([len(sent) for sent in encoded_questions])
#     #print(max_len)
#     return max_len
...
#         batch_size = out5.shape[0]
#         embedding_dim_size = out5.shape[1]
#         image_feature = out5.view(batch_size, embedding_dim_size, -1) # [N, 512, 196]
...
class VqaClassifierModel(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # self.Linear_predict1 = nn.Linear(self.opt.MFB_OUT_DIM, 2000)
        # self.Linear_predict2 = nn.Linear(2000, 1024)
        # self.Linear_predict3 = nn.Linear(1024, self.opt.MFB_OUT_DIM)
    ...
    def forward(self, img, qst):
        ...
        # fc1 = self.Linear_predict1(mfb_l2)
        # fc2 = self.Linear_predict2(fc1)
        # fc3 = self.Linear_predict3(fc2)
        prediction = self.Linear_predict(mfb_l2) #self.Linear_predict(fc3)
